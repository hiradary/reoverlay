...
const isFile = (fieldPath, relativePath) => {
  ...
  const filePathExists = getNodesByType(`File`).some(node => node.absolutePath === filePath);
...
const getFilePath = (fieldPath, relativePath) => {
  ...
  const node = getNodesByType(typeName).find(node => getFirstValueAt(node, selector) === normalizedPath);
