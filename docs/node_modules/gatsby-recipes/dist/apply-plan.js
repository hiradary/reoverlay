...
const applyPlan = async stepPlan => {
  let appliedResources = []; // We apply each resource serially for now — we can parallelize in the
  // future for SPEED
  ...
  await asyncForEach(stepPlan, async resourcePlan => {
    ...
    try {
      const changedResources = await resource.create(ctx, resourcePlan.resourceDefinitions);
